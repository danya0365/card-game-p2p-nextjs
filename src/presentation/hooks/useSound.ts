...
export function useSound() {
  ...
  const playBet = useCallback(() => soundService.play("bet"), []);
  const playChip = useCallback(() => soundService.play("chip"), []);
  const playDeal = useCallback(() => soundService.play("deal"), []);
  const playShuffle = useCallback(() => soundService.play("shuffle"), []);
